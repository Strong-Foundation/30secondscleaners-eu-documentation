...
func main() {
	remoteAPIURL := "https://30secondscleaners.eu/30-seconds-safety-data-sheets/" // URL to fetch HTML content from
	...
	if !fileExists(localFilePath) { // Check if local HTML file already exists
		getData = scrapePageHTMLWithChrome(remoteAPIURL) // If not, download HTML content from URL
	...
	for _, urls := range finalList {
		domainPrefix := "https://30secondscleaners.eu"
		if !strings.HasPrefix(urls, domainPrefix) {
			urls = domainPrefix + urls
		}
